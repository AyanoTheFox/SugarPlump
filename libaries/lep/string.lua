...
function string.getChars(_str)
    local _t = {}
    for c in string.gmatch(_str, ".") do table.insert(_t, c) end
    return _t
end
...
function string.matchp(_str, _match) for _s, _m, _f in string.gmatch(_str, '()('.. _match .. '+)()[^' .. _match .. ']*$') do return tonumber(_s), tonumber(_f) - 1, _m end end
